...
from org.betterxml.util.UserStack import UserStack
...
class XAttributes(UserDictObject):
    ...
    def __setitem__(self, key, item):
        if type(key) != types.TupleType and len(key) != 2:
            raise "key must be a pair (namespace, name)"
...
class XElement(object):
    ...
    def linkTo(self, element):
        if isinstance(element, XElement):
            ...
            raise "An XElement is required; a %s was specified." % type(element)
...
class XTreeHandler(ContentHandler, ErrorHandler):
    ...
    def registerNamespace(self, userModule):
        ...
        if type(userModule) != types.ModuleType:
            raise "a module must be specified"
        ...
        if callable(getNamespaceURI):
            ...
            raise "%s.getNamespaceURI() missing or not callable" % moduleName
        
        if moduleURI in self.namespaceModules.keys():
            raise "%s.getNamespaceURI() duplicate NS definition %s" % (moduleName, moduleURI)
        ...
        if callable(getDefaultElementClass):
            ...
            raise "module must provide getDefaultElementClass() definition"

        if not type(xeClass) == type(XElement) or not issubclass(xeClass, XElement):
            raise "getDefaultElementClass() in %s must return XElement subclass" % moduleName
           
        getMappings = getattr(userModule, "getMappings", None)
        if callable(getMappings):
            mappings = getMappings()
        else:
            raise "%s.getMappings() definition missing or not callable"
        
        mappings = getMappings()
        for (elementName, elementClass) in mappings.items():
            if type(elementName) != type(''):
                raise "%s.getMappings() has dict with non-string key" % moduleName
            if not type(xeClass) == type(XElement) or \
               not issubclass(xeClass, XElement):
                raise "%s.getMappings() has dict with non-XElement value" % moduleName
    ...
    def startElementNS(self, name, qname, attrs):
        ...
        try:
            ...
            raise "unlikely error; unmapped namespace URI %s: Forgot to register it?" % ns_uri
        ...
        if elementClass == None:
            raise "Unable to map element %s in URI %s to a default or user-defined class" % \
                  (element_name, ns_uri)
